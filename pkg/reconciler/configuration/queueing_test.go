...
func getTestConfiguration() *serving.Configuration {
	cfg := &serving.Configuration{
		ObjectMeta: metav1.ObjectMeta{
			SelfLink:  "/apis/serving/serving/namespaces/test/configurations/test-config",
			Name:      "test-config",
	...
	// TODO(dprotaso) Figure out defaulting
...
func TestNewConfigurationCallsSyncHandler(t *testing.T) {
	...
	if _, err := servingClient.Serving().Configurations(config.Namespace).Create(config); err != nil {
