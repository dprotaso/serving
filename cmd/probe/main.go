...
import (
	...
	"knative.dev/serving/pkg/queue/constants"
)
...
func main() {
	readinessProbeTimeout, _ := time.ParseDuration(os.Args[1])
...
func probeQueueHealthPath(timeout time.Duration, queueServingPort int, transport http.RoundTripper) error {
	...
	req.Header.Add(ProbeHeaderName, constants.Name)
	req.Header.Add(UserAgentKey, QueueProxyUserAgent)
