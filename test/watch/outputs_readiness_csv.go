...
func getReplicaReadyTime(history *ObjectHistory) (string, *unstructured.Unstructured, bool) {
	...
	for _, event := range history.K8sEvents {

		name, _, _ := unstructured.NestedString(event.Object, "reason")

		if name != "SuccessfulCreate" {
			continue
		}

		timestamp, _, _ := unstructured.NestedString(event.Object, "lastTimestamp")
		return timestamp, obj, true
	}
	return "", nil, false
}
