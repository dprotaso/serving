...
func (o *JaegerTraces) WriteHistory(h GVRHistory) {
	...
	for _, root := range graphRoots {
		o.createSpan(root, root.uid, rss)
	}
...
func (o *JaegerTraces) createSpan(n *node, traceid string, rss ptrace.ResourceSpansSlice) {
	name := NameExtractorRegexp.FindString(n.name)
	rspans := rss.AppendEmpty()

	r := rspans.Resource()
	attr := r.Attributes()
	attr.UpsertString(conventions.AttributeServiceName, "scale")
	...
	if !ok {
		...
		fmt.Println("skipping since is not ready", n.gvr, name)
		return
	}
	...
	for _, child := range n.children {
		o.createSpan(child, traceid, rss)
	}
}
